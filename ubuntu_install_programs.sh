sudo apt update -y && sudo apt upgrade -y

sudo apt install cmus opam gcc gfortran texlive-full vim \
clisp chezscheme golang-go openmpi-bin libopenmpi-dev \
youtube-dl vlc zathura stterm sl lolcat yakuake \
lollypop clementine transmission-gtk calibre ranger \
libsdl2-ttf-dev libsdl2-dev blender freeglut3 freeglut3-dev \
gimp krita opengl-4-man-doc tmux fmit cmake ghc \
python3-pip libsdl2-image-dev r-base-core wine krita \
screenfetch gcc-multilib g++-multilib htop eog -y

